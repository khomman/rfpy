...
def test_set_hkgrid():
    ...
    hk = HKStack(st, depth_range=(30,40), depth_inc=1, kappa_range=(1.6, 2.0),
                kappa_inc=0.1)
    depths = hk.depths
    kappas = hk.kappas
    gridh = hk.hh
    gridk = hk.kk
    assert [depths[0], depths[-1], kappas[0], np.round(kappas[-1], 1), len(depths),
            len(kappas)] == [30, 39, 1.6, 1.9, 10, 4]
