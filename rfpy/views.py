...
def index():
    ...
    status = {'total': total_sta, 'todo': todo, 'hk': hk, 'qc': qc,
                'qcpercent': int(100*(qc/total_sta)), 'hkpercent': int(hk/total_sta)}
    ...
    if request.method == 'POST':
        ...
        if request.form['selectAll'] == 'yes':
            ...
            rf_query = db.session.query(ReceiverFunctions).join(Stations).join(
                                Filters).filter(Stations.station==sta).filter(
...
def hkstack():
    ...
    if request.method == 'POST':
        ...
        rf_query = db.session.query(ReceiverFunctions).join(
                                    ...
                                    Filters.filter==filt)
        print(sta, filt)
        rfs = [rftn.path for rftn in rf_query if rftn.accepted==True and rftn.path[-1] == 'r']
        if len(rfs)==0:
            flash('No accepted receiver functions!  Add receiver functions and perform quality control')
...
def stationmap():
    ...
    ax = station_map(sta_lats, sta_lons, sta_names=sta_names,
                filename=os.path.join(app.root_path, 'static', 'station_map.svg'))
...
def hkmap():
    ...
    return render_template('plots.html', plot=[depth_plot, kappa_plot], format='hk')
