...
class HKStack:
	...
	chapter 4 of Philip Crotwells dissertation for a description of Phase weight
	...
	def __init__(self, stream, station=None, vp=6.2, depth_range=(32,50),
		...
		w2=0.2, w3=0.1, pws=False, bs=False, bs_replications=200, nell=250,
		starttime=-2,endtime=30, plotfile='hkstack.svg'):
		"""
	...
	def _do_hkstack(self,is_bs=False):
        ...
        #correct then adjust code. and PWS code.
			...
			rf1 = tr.data[t1] + (tr.data[(t1+1)] - tr.data[t1]) * (time_Ps - beg -
		    	(t1-0) * delta)/delta
			rf2 = tr.data[t2] + (tr.data[(t2+1)] - tr.data[t2]) * (time_PpPs - beg -
		    	(t2-0) *delta)/delta
			rf3 = tr.data[t3] + (tr.data[(t3+1)] - tr.data[t3]) * (time_PpSs - beg -
				(t3-0) * delta)/delta
